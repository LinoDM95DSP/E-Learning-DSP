...
class ExecutePythonCodeView(APIView): # Ändere Vererbung zu APIView
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Prepare execution environment and capture output/results
        ...
        test_results = None
        test_results_obj = None # Initialisieren

        try:
            # 1. Execute the user's code (UNSAFE!)
            # The user's code might define functions/classes the tests expect
            ...
            execution_error = f"Error during code execution or testing: {e}"
        finally:
            sys.stdout = old_stdout
            sys.stderr = old_stderr # Restore stderr

        output = redirected_output.getvalue()
        errors = redirected_error.getvalue() # Capture stderr as well

        response_data = {
            'stdout': output,
            'stderr': errors, # Include stderr
            'execution_error': execution_error, # Report errors from the try/except block
